...
def mean_values_map(ds, var, model, mode,  diff = None, months=None, season='Annual',
    ...
    # If a specific season is selected, filter the dataset by the corresponding months
    #if season != 'Annual':
    #    seasons = {
    #        'DJF': [12, 1, 2],
    #        'MAM': [3, 4, 5],
    ...
    #    ds = ds.sel(time=ds['time.month'].isin(selected_months))
...
def categories_robustness(ds, var, months = None, season = 'Annual', period=slice('2081', '2100'),
                          baseline_period=slice('1981', '2010'), GWLs_ds = None):
    '''
    Divides the provided data into 3 categories of robustness for each latitude and longitude.
    ...
    season : str, optional
        The season for which to calculate the mean ('Annual', 'DJF', 'MAM', 'JJA', 'SON'). Default is 'Annual'.
    period : slice, optional
        The period for which to calculate robustness. Default is slice('2081', '2100').
    baseline_period : slice, optional
        The baseline period for calculating the mean baseline. Default is slice('1981', '2010').
...
def annual_weighted_average(ds,var, season = 'Annual', months = None, 
