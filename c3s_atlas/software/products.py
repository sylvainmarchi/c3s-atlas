...
def hatched_map_plot(ds, var, attrs, mode, diff = None, categories = None, period = None,
    ...
    if mode == "trends":
        ...
        min_values = - max_values
        p = (ds['slope']*10).plot(cmap = cmap, vmin = min_values, vmax = max_values, 
                                  ax = ax, cbar_kwargs={"label": f"Units: {attrs.get("unit")} per decade"})
...
def annual_cycle(ds, var, attrs, mode, diff = None, 
    ...
    if attrs.get("project") in ["ERA5", "ERA5-Land", "E-OBS"]:# project that don't have menber
        ...
        ax.plot([], color='red', linewidth=0.25, linestyle = ":" ,label = 'project')# to add it to the legend
...
def time_series(ds, var, attrs, mode = 'climatology', diff = 'abs', 
   ...
   #set title
    ...
    # Add legend
    ax.plot([], color='red', linewidth=0.5,linestyle = ':', label= 'Project')
    ax.legend()
